...
const OurBooks = () => {
  ...
  useEffect(() => {
    AOS.init({
      ...
      offset: 200 // Change offset to trigger animations sooner or later (px)
    });
  }, []);
      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 place-items-center gap-5">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="animate-pulse p-7 rounded-2xl shadow-lg max-md:px-5 w-[200px] h-[350px] ml-1 bg-gray-200"></div>
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="container mt-28">
      <Breadcrumb />
      <div className="text-center mb-10 max-w-[400px] mx-auto" data-aos="slide-up">
        <h1 className="text-3xl font-bold text-indigo-800">Our Books</h1>
        <p className="text-xs mt-4 text-indigo-800 opacity-75">
          Discover some of the most influential books that have shaped minds and inspired generations.
        </p>
